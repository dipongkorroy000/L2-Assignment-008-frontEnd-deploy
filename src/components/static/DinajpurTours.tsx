  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-100 via-white to-blue-50 py-20">
      ...
      <section className="text-center py-16">
        ...
        <p className="mt-4 text-gray-600">Explore history, culture, and nature in Dinajpur for an unforgettable journey.</p>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-3 gap-8">
        {places.map((place, i) => (
          ...
          >
            <Image src={place.img} alt={place.name} width={400} height={250} className="w-full h-56 object-cover" />
            <div className="p-6">
              <h2 className="text-xl font-semibold text-blue-700">{place.name}</h2>
              <p className="text-gray-600 mt-2">{place.description}</p>
              <button className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Find Guides</button>
            </div>
          </motion.div>
        ))}
      </section>
