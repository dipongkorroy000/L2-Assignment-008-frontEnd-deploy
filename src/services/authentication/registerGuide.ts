...
export const registerGuide = async (_currentState: any, formData: any): Promise<any> => {
  try {
    const payload = {
    ...
    const validatedPayloadData = {
      ...
      guide: {name: validatedPayload.name, contactNumber: validatedPayload.contactNumber, email: validatedPayload.email},
    };

    const res = await server_fetch.post("/user/create-guide", {body: JSON.stringify(validatedPayloadData), headers: {"Content-Type": "application/json"}});
